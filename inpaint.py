from inpaint_args import TASK_PARAMS
...
from modules.patch import patch_settings, PatchSettings, patch_all
...
patch_all()
modules.config.default_base_model_name = "realisticStockPhoto_v20.safetensors"
import modules.default_pipeline as pipeline  # NOQA: E402

# Mock the input
image_args = TASK_PARAMS.args[35]
assert "image" in image_args and "mask" in image_args

image = imread("sample_image.png")
mask = imread("mask_input.png")
...
TASK_PARAMS.performance_selection = flags.Performance.SPEED
    ...
    for task in async_task.cn_tasks[flags.cn_canny]:
        ...
        if not async_task.skipping_cn_preprocessor:
            cn_img = preprocessors.canny_pyramid(
                cn_img, async_task.canny_low_threshold, async_task.canny_high_threshold
            )
...
def save_and_log(
    async_task, height, imgs, task, use_expansion, width, loras, persist_image=True
) -> list:
    img_paths = []
    for x in imgs:
        d = [
        ...
        d.append(
            (
                ...
                "metadata_scheme",
                (
                    async_task.metadata_scheme.value
                    if async_task.save_metadata_to_images
...
def process_task(
    ...
    img_paths = save_and_log(
        async_task, height, imgs, task, use_expansion, width, loras, persist_image
    )
    yield_result(
        async_task,
        img_paths,
...
def apply_inpaint(
    ...
    if async_task.debugging_inpaint_preprocessor:
        ...
        raise BaseException("Early Return")  # Not coding another class here
    ...
    print(
        f"Final resolution is {str((final_width, final_height))},",
        f" latent is {str((width, height))}.",
    )
...
def apply_upscale(
    ...
    initial_latent = core.encode_vae(vae=candidate_vae, pixels=initial_pixels, tiled=True)
...
def prepare_enhance_prompt(prompt: str, fallback_prompt: str):
    if (
        safe_str(prompt) == ""
        or len(remove_empty_str([safe_str(p) for p in prompt.splitlines()], default="")) == 0
    ):
...
def process_enhance(
    ...
    if "upscale" in goals:
        (
            ...
            height,
            current_progress,
        ) = apply_upscale(async_task, img, async_task.enhance_uov_method, switch, current_progress)
        if direct_return:
...
def apply_patch_settings(task):
    patch_settings[os.getpid()] = PatchSettings(
        task.sharpness,
        task.adm_scaler_end,
...
def apply_image_input(
    ...
    if (
        (
            async_task.current_tab == "uov"
            or (async_task.current_tab == "ip" and async_task.mixing_image_prompt_and_vary_upscale)
        )
        and async_task.uov_method != modules.flags.disabled.casefold()
    ...
    if (
        async_task.current_tab == "inpaint"
        or (async_task.current_tab == "ip" and async_task.mixing_image_prompt_and_inpaint)
    ) and isinstance(async_task.inpaint_input_image, dict):
        inpaint_image = async_task.inpaint_input_image["image"]
        inpaint_mask = async_task.inpaint_input_image["mask"]

        if async_task.inpaint_advanced_masking_checkbox:
            if isinstance(async_task.inpaint_mask_image_upload, dict):
                if (
                    isinstance(async_task.inpaint_mask_image_upload["image"], np.ndarray)
        ...
        if (
            ...
            if inpaint_parameterized:
                ...
                inpaint_head_model_path, inpaint_patch_model_path = (
                    modules.config.downloading_inpaint_models(async_task.inpaint_engine)
                )
                base_model_additional_loras += [(inpaint_patch_model_path, 1.0)]
...
def handler(async_task):
    ...
    if "upscale" in goals:
        ...
        if direct_return:
            ...
            uov_input_image_path = log(
                async_task.uov_input_image, d, output_format=async_task.output_format
            )
            yield_result(
                async_task,
    ...
    if "inpaint" in goals:
        ...
        except Exception as err:  # not coding a special class
            raise err
    ...
    for index, img in enumerate(images_to_enhance):
        ...
        if enhance_uov_before:
            current_task_id += 1
            persist_image = (
                not async_task.save_final_enhanced_image_only or active_enhance_tabs == 0
            )
            (
                current_task_id,
                done_steps_inpainting,
        ...
        for (
            ...
            persist_image = (
                not async_task.save_final_enhanced_image_only or is_last_enhance_for_image
            )
            ...
            try:
                ...
                if (
                    ...
                    and async_task.enhance_uov_processing_order == flags.enhancement_uov_after
